...
def is_greek_char(char):
                import re
                return bool(re.match(r'([\u0373-\u03FF]|[\u1F00-\u1FFF]|\u0300|\u0301|\u0313|\u0314|\u0345|\u0342|\u0308)',char,re.UNICODE))
...
def split_text_token(stringIn):
    import re
    superscripts = '\u00B2\u00B3\u00B9\u2070\u2074\u2075\u2076\u2077\u2078\u2079'
    word_parts = re.match('(^[\d' + superscripts + '·»«„\*\[\("〈,-\.\?’|‖“⟨]*)(.*?)([⟩”«»„.,!?;†·:〉\'\(\)\d\]\[|' + superscripts + ']*$)',stringIn,re.UNICODE)
    try:
...
def preprocess_word(word):
    ...
    word = re.sub(r'^' + apostrophe + r'([I1ΙιΕEAP])(.*)$', r'\1' + smooth_breathing+ r'\2', word)
