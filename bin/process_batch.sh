...
#	    eval $CIACONNA_HOME/bin/ocropus_page.sh $columns_command $migne_command  $binarization_threshold  -l $CLASSIFIER_FILE -o $HOCR_OUTPUT_DIR/$filebase.html $image_file 
	 ...
	 if [ "$processor" == "ocropus" ]; then
		...
		parallel  -P $NUMBER_OF_CORES "tesseract   -l lat+Greek  {}  $HOCR_OUTPUT_DIR/{/.} hocr" ::: *png
		...
		parallel --timeout 600  -P $NUMBER_OF_CORES "kraken  -v -h -d cuda:0  -i {} $DATE/{.}.html binarize --threshold ${binarization_threshold##* } segment  --maxcolseps 1  -r  $columns_command   ocr  -m $CLASSIFIER_FILE" ::: *png 
